		...
		for _, path := range []string{"responses", "passthrough", "chat", "messages"} {
			for _, carrier := range []string{"header", "metadata"} {
				t.Run(accountType+"/"+path+"/"+carrier, func(t *testing.T) {
					...
					seed, err := prepareCodexStateTest(state, context.Background(), account, "gpt-5.4")
					...
					state.collector = NewCodexTurnStateHTTPCollector(func(_ context.Context, input CodexTurnStateCollectRequest, request *http.Request) (*http.Response, error) {
						...
						input.CaptureBundleBinding(CodexTurnStateBundleBinding{WireMode: "lite", EgressKind: "proxy", ProxyID: 2, ProxyRouteGeneration: 1})
						require.Empty(t, request.Header.Get(openAICodexTurnStateHeader))
						body, readErr := io.ReadAll(request.Body)
						require.NoError(t, readErr)
						require.Contains(t, gjson.GetBytes(body, "instructions").String(), "pong")
					...
					gateway.SetCodexTurnStateProxyRepository(codexCollectorTransportProxies{proxy: &Proxy{ID: 2, Host: "127.0.0.1", Port: 7897, Protocol: "http", Status: StatusActive, RouteGeneration: 1}})
					...
					require.NoError(t, err)
					require.NotEmpty(t, recovered.EncryptedToken, "collector reason: %s", recovered.LastError)
