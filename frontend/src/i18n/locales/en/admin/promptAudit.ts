export default {
  promptAudit: {
    ...
    keyword: { category: 'Keyword block', description: 'The prompt matched an administrator-configured keyword rule.', localPolicy: 'Local keyword policy' },
    runtime: {
      ...
      queue: 'Active jobs / capacity', dependencies: 'Dependencies', keywordBlocking: 'Keyword blocking', guardMetrics: 'Audit metrics', latest: 'Latest processing and error',
    ...
    metrics: { total: 'Total', allowed: 'Allowed', flagged: 'Flagged', blocked: 'Blocked', keywordBlocked: 'Keyword blocks', unavailable: 'Unavailable', timeouts: 'Timeouts', failovers: 'Failovers' },
    pool: {
    ...
    policy: {
      title: 'Audit policy', description: 'Configure separate Guard and keyword scopes, risk categories, workers, and queue bounds.', scope: 'Scope', guardScope: 'Guard audit scope', keywordScope: 'Keyword blocking scope', allGroups: 'All groups', selectedGroups: 'Selected groups',
      searchGroups: 'Search groups', searchGuardGroups: 'Search Guard audit groups', searchKeywordGroups: 'Search keyword blocking groups', noGroups: 'No matching groups', missingGroups: 'Configured IDs for groups that no longer exist', removeMissingGroup: 'Remove deleted group {id}', selectedCount: '{count} groups selected',
      scanners: 'Qwen3Guard input-risk categories', keywordTitle: 'Keyword blocking rules', keywordDescription: 'Independent of Guard nodes; a match rejects the request immediately.', keywords: 'Keywords (one per line)', keywordsPlaceholder: 'Example: internal secret', keywordCount: '{count} configured', keywordLimit: 'Up to 10,000 keywords, 200 characters each', workerCount: 'Worker count', queueCapacity: 'Persistent queue capacity', strategy: 'Node strategy', strategyHint: 'Try nodes in configuration order and fail over when allowed.',
    },
    saveBar: { enabled: 'Enable Guard audit', blocking: 'Guard synchronous blocking', keywordBlocking: 'Keyword blocking', blockingLatestTurnOnly: 'Only latest input and prior output', storePass: 'Store safe events', dirty: 'Unsaved changes', synced: 'Configuration synced' },
    blockingConfirm: {
    ...
    events: {
      ...
      guardReturn: 'Audit result',
      guardReturnHint: 'Structured result from the model or local keyword policy. Raw response bodies are not stored.',
    ...
    errors: {
      ...
      prompt_guard_requires_audit_enabled: 'Enable Guard audit before synchronous blocking.', prompt_audit_invalid_endpoint: 'The audit node configuration is invalid.', prompt_audit_endpoint_required: 'Enable at least one audit node before enabling Guard audit.', prompt_audit_groups_required: 'Select at least one Guard group in selected-group mode.', prompt_audit_keyword_groups_required: 'Select at least one keyword blocking group in selected-group mode.', prompt_audit_invalid_keyword_group: 'The keyword blocking group ID is invalid.', prompt_audit_scanners_required: 'Enable at least one risk category.', prompt_audit_keywords_required: 'Configure at least one keyword before enabling keyword blocking.', prompt_audit_too_many_keywords: 'The keyword count exceeds the allowed limit.', prompt_audit_keyword_too_long: 'A keyword exceeds the allowed length.',
    },
  },
}
