...
const bulkTargets = computed(() => {
  ...
  return {
    extend: selected.filter((subscription) => ['active', 'expired'].includes(subscription.status) && !isTotalQuotaSubscription(subscription)),
    reset_quota: selected.filter((subscription) => subscription.status === 'active' && !isTotalQuotaSubscription(subscription)),
