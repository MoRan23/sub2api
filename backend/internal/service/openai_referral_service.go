...
func (s *OpenAIQuotaService) referralCall(ctx context.Context, id int64, program string) (context.Context, OpenAIReferralCall, error) {
	if s.referralClient == nil {
		return ctx, OpenAIReferralCall{}, infraerrors.New(http.StatusServiceUnavailable, "OPENAI_REFERRAL_NOT_CONFIGURED", "referral service is unavailable")
	}
	ctx = FreezeOpenAIRequestPolicy(ctx, s.settingService)
	prepared, err := s.prepareUpstreamCall(ctx, id)
	if err != nil {
		return ctx, OpenAIReferralCall{}, err
	}
	ctx = prepared.ctx
	headers, _, err := s.buildCodexQuotaHeaders(ctx, id, prepared.accessToken, prepared.chatGPTAccountID, prepared.fedRAMP)
	if err != nil {
		return ctx, OpenAIReferralCall{}, infraerrors.New(http.StatusBadGateway, "OPENAI_REFERRAL_AUTH_ERROR", "failed to authenticate referral request")
	}
	return ctx, OpenAIReferralCall{ProxyURL: prepared.proxyURL, Headers: headers, ProgramID: program}, nil
}
...
func (s *OpenAIQuotaService) QueryReferralEligibility(ctx context.Context, id int64) (*OpenAIReferralEligibility, error) {
	...
	callCtx, call, err := s.referralCall(callCtx, id, program)
	if err != nil {
		return nil, err
	}
...
func (s *OpenAIQuotaService) SendReferralInvite(ctx context.Context, id int64, input OpenAIReferralSendRequest) (*OpenAIReferralSendResult, error) {
	...
	callCtx, cancel := context.WithTimeout(ctx, openaiQuotaUpstreamTimeout)
	defer cancel()
	callCtx, call, err := s.referralCall(callCtx, id, eligibility.ProgramID)
	if err != nil {
		return nil, err
	}
