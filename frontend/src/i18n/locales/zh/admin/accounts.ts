export default {
    accounts: {
      codexTurnState: {
        ...
        description: '真实业务触发并行采集，票据与 Cookie 包按凭据账号及最终模型跨系统复用；仅维护系统生效名单内的模型，无缓存时照常转发。关闭后不再池化、替换或保存 Cookie。',
        accountType: '账号分类',
        ...
        proxyHint: '后台短请求按列表顺序使用独立代理，业务继续使用原代理。空列表仅从业务响应学习；正常有效缓存不会因修改列表立即重采。',
        ...
        lifetimePolicyHint: '票据本地有效期从签发起 240 秒；活跃模型成功后每 30 秒继续采集，单次上限 12 秒，近 30 分钟无业务则停止。票据包按票据与 Cookie 的最早到期时间复用；这些是本项目策略，不代表上游已确认的有效期。',
        loading: '正在加载状态…',
        ...
        cookieSources: { none: '未携带', bundle: '本次票据包', persistent: '持久化池', memory: '进程会话池', mixed: '持久化与进程会话池', unknown: '未记录' },
        cookieReasons: {
        ...
        reasons: {
          ...
          business_active: '真实业务已触发采集', refresh: '按活跃模型计划持续刷新',
        ...
        reasonHints: {
          ...
          refresh: '上次成功后等待 30 秒继续采集；该模型近 30 分钟无业务时停止，不属于错误退避。',
