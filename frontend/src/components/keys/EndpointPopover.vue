...
<template>
  ...
  >
    <div
      ...
      :key="index"
      class="relative flex w-full min-w-0 max-w-full items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-2.5 py-1.5 text-xs transition-colors hover:border-primary-200 sm:w-auto dark:border-dark-600 dark:bg-dark-800 dark:hover:border-primary-700"
      data-test="endpoint-item"
    >
      <span
        class="max-w-24 shrink-0 truncate font-medium text-gray-600 sm:max-w-40 dark:text-gray-300"
        :title="item.name"
      >{{ item.name }}</span>
      <span
        v-if="item.isDefault"
      ...
      <div class="group/endpoint flex min-w-0 flex-1 items-center gap-1.5">
        <div
          class="pointer-events-none absolute bottom-full right-0 z-20 mb-2 w-max max-w-[min(24rem,100%)] translate-y-1 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-left opacity-0 shadow-[0_14px_36px_-20px_rgba(15,23,42,0.35)] ring-1 ring-slate-200/80 transition-all duration-150 group-hover/endpoint:translate-y-0 group-hover/endpoint:opacity-100 group-focus-within/endpoint:translate-y-0 group-focus-within/endpoint:opacity-100 dark:border-slate-700 dark:bg-slate-900 dark:ring-slate-700/70"
          data-test="endpoint-tooltip"
        >
          <p
