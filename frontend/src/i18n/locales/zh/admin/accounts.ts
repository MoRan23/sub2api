export default {
    accounts: {
      codexTurnState: {
        ...
        proxyRotationHint: '每模型累计三次有效异常形态后轮换；个人仅累计 312／11 块，Team / Business 仅累计 356／13 块。其他错误不计数也不清零；取得满足需求的目标状态后清零并保留当前代理。轮换后仍遵守至少 30 秒及更长冷却的重试间隔。',
        ...
        reasonHints: {
          ...
          no_target_state: '响应没有提供可用目标状态，可能缺失或形态、签发时间不合要求。可对照右侧最近观测。',
          target_still_expiring: '返回状态仍处于临期窗口，未完成续期；旧缓存尚有效时可继续使用。',
