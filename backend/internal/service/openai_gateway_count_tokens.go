...
func (s *OpenAIGatewayService) buildInputTokensUpstreamRequest(
	...
	if account.UsesOpenAICodexProtocol() {
		...
		if planErr != nil {
			return nil, fmt.Errorf("resolve responses input_tokens OAuth profile identity: %w", planErr)
		}
		// The pure projector intentionally does not invent an originator for
		// compatibility builders. Seed it from the same immutable profile plan,
		// then project the strict TUI triplet last so inbound UA cannot win.
		req.Header.Set("originator", profilePlan.ClientIdentity.Originator)
		if _, applyErr := ApplyOpenAIOAuthIdentityPlan(req.Header, nil, profilePlan); applyErr != nil {
			return nil, fmt.Errorf("apply responses input_tokens OAuth profile identity: %w", applyErr)
		}
	}
