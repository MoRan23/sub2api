...
// openAIWSFinalizingUpstreamFrameConn owns the final client-to-upstream write
...
func (c *openAIWSFinalizingUpstreamFrameConn) ReadFrame(ctx context.Context) (coderws.MessageType, []byte, error) {
	if c == nil || c.inner == nil {
		return coderws.MessageText, nil, errOpenAIWSConnClosed
	}
...
func (c *openAIWSFinalizingUpstreamFrameConn) WriteFrame(ctx context.Context, msgType coderws.MessageType, payload []byte) error {
	...
	if c.finalize != nil {
		finalized, err := c.finalize(msgType, payload)
		if err != nil {
			return err
		}
		payload = finalized
...
func (s *OpenAIGatewayService) proxyResponsesWebSocketV2Passthrough(
	...
	for {
		...
		upstreamConn, statusCode, handshakeHeaders, err = dialer.Dial(withOpenAIWSTransportAccount(dialCtx, account.ID), wsURL, headers, proxyURL)
		cancelDial()
		if err == nil {
			break
		}
	...
	policyClientConn := &openAIWSPolicyEnforcingFrameConn{
		...
		filter: func(msgType coderws.MessageType, payload []byte) (out []byte, blocked *OpenAIFastBlockedError, filterErr error) {
			...
			if isResponseCreate {
				// OAuth-like follow-up payloads remain in their non-Lite shape until
				// the same final capability decision used by the first physical write.
			...
			if account.UsesOpenAICodexProtocol() && isResponseCreate {
	...
	readNextClientFrame := func(readCtx context.Context, conn openaiwsv2.FrameConn) (coderws.MessageType, []byte, error) {
		for {
			...
			if writeErr := physicalUpstreamFrameConn.WriteFrame(readCtx, msgType, payload); writeErr != nil {
	...
	relayResult, relayExit := openaiwsv2.RunEntry(openaiwsv2.EntryInput{
		...
		UpstreamConn:       physicalUpstreamFrameConn,
		...
		Options: openaiwsv2.RelayOptions{
			...
			BeforeWriteClient: func(msgType coderws.MessageType, payload []byte, wroteDownstream bool) error {
				...
				return s.newOpenAIWSSemanticRateLimitFailoverError(account, handshakeHeaders, payload, errMsgRaw)
			},
