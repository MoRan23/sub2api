export default {
    ...
    riskControl: {
      ...
      cyberPolicyWhitelist: 'cyber_policy 白名单提醒',
      cyberPolicyWhitelistHint: '白名单用户命中 cyber_policy 时仍记录，但不计入自动封号；邮件会发送给用户邮箱和下方配置的邮箱。',
      cyberPolicyWhitelistUsers: '白名单用户',
      cyberPolicyNotificationEmails: '额外通知邮箱',
      cyberPolicyNotificationEmailPlaceholder: '输入邮箱后添加',
      cyberPolicyAddNotificationEmail: '添加邮箱',
      cyberPolicyRemoveNotificationEmail: '移除通知邮箱',
      cyberPolicyInvalidNotificationEmail: '请输入有效的通知邮箱',
      cyberPolicyNotificationEmailsHint: '未选中审计分组的 cyber_policy 命中不会发送通知。',
      ...
      groupScope: '审计分组',
      groupScopeHint: '开启右侧开关表示全部分组，关闭后选择指定分组；该范围同时作用于内容审核和 cyber_policy 相关记录。',
      selectedGroups: '指定分组',
      searchGroups: '搜索分组名称或平台',
      noGroups: '暂无可用分组',
      contentModerationWhitelist: '审核放行用户',
      contentModerationWhitelistHint: '这些用户的请求仍会异步接受内容审核并记录命中，但始终立即放行，且不会累计违规或自动封禁。提示词审计和网络安全策略不受影响。',
      contentModerationWhitelistUsers: '放行用户',
      modelFilter: '模型范围',
