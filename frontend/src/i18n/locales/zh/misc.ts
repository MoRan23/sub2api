export default {
  ...
  customPage: {
    ...
    purchaseGuideUsdtNotice: '如需使用USDT支付请使用站内充值/订阅处',
    goToRedeem: '去兑换页',
    notFoundTitle: '页面不存在',
  ...
  userSubscriptions: {
    ...
    monthly: '每月',
    total: '总额',
    expiringQuotaHint: '${amount} 额度将在 {time} 后到期',
    daysRemaining: '剩余 {days} 天',
    expiresOn: '{date} 到期',
    resetIn: '{time} 后重置',
  ...
  payment: {
    ...
    admin: {
      ...
      confirmRefund: '确认退款',
      refundSuccess: '退款成功',
      refundSuccessWithDeductions: '退款成功，已回收普通余额 {ordinary}、赠送余额 {gift}，合计 {total}',
      ...
      alreadyRefunded: '已退款',
      deductBalance: '扣除余额',
      deductBalanceHint: '按退款比例从普通余额和赠送余额分别扣回',
      userBalance: '用户余额',
      userOrdinaryBalance: '可用普通余额',
      userGiftBalance: '可用赠送余额',
      loadingWallet: '正在读取用户钱包余额...',
      orderAmount: '订单金额',
      insufficientBalance: '余额不足，将扣至 $0',
      ordinaryBalanceInsufficient: '普通余额不足：可用 ${available}，需回收 ${required}',
      giftBalanceInsufficient: '赠送余额不足：可用 ${available}，需回收 ${required}',
      estimatedOrdinaryRecovery: '预计回收普通余额',
      estimatedGiftRecovery: '预计回收赠送余额',
      estimatedTotalRecovery: '预计合计回收',
