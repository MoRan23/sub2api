...
func (s *AccountRepoSuite) TestOAuthOSCredentialsMigrationDefaultOnlyAndNoResurrection() {
	...
	s.Require().Equal(service.OpenAIOAuthAuthorizationUnauthorized, profiles.Profiles[service.OpenAIOSWindows].Authorization.Status)
...
func (s *AccountRepoSuite) TestOAuthOSCredentialsBindDefaultAndStaleSnapshots() {
	...
	s.Require().NoError(err)
	_, err = s.repo.BindOpenAIOAuthOSCredentials(s.ctx, account.ID, service.OpenAIOSMacOS, oauthOSTestGrant("linux"), "test")
	s.Require().ErrorIs(err, service.ErrOpenAIOAuthOSRefreshTokenReused)
...
func (s *AccountRepoSuite) TestOAuthOSCredentialsCASRejectsRevokeAndConcurrentRefresh() {
	...
	for i := 0; i < 2; i++ {
		...
		go func() {
			defer wg.Done()
			ok, err := s.repo.PatchOpenAIOAuthOSCredentialsIfUnchanged(context.Background(), account.ID, service.OpenAIOSWindows, slot.AuthorizationGeneration, slot.Revision, nil, map[string]any{"access_token": "refreshed"}, nil)
			successes <- ok
			failures <- err
		}()
	}
...
func (s *AccountRepoSuite) TestOAuthOSCredentialsInitialImportIsAtomic() {
	...
	s.Require().Equal(service.OpenAIOAuthAuthorizationUnauthorized, account.OpenAIOAuthOSProfiles.Profiles[service.OpenAIOSWindows].Authorization.Status)
}
...
func (s *AccountRepoSuite) TestOAuthOSCredentialsSlotErrorsAndDefaultRevokeStayIsolated() {
	...
	fresh, err := s.repo.GetByID(s.ctx, account.ID)
	s.Require().NoError(err)
	s.Require().Equal(service.OpenAIOAuthAuthorizationAuthorized, fresh.OpenAIOAuthOSProfiles.Profiles[service.OpenAIOSLinux].Authorization.Status)
	...
	s.Require().NoError(err)
	s.Require().Equal(service.OpenAIOSLinux, fresh.OpenAIOAuthOSProfiles.DefaultOS)
	_, err = service.ResolveOpenAIOAuthCredentialAccount(s.ctx, s.repo, fresh, "")
	s.Require().ErrorIs(err, service.ErrOpenAIOAuthOSUnauthorized)
	_, err = service.ResolveOpenAIOAuthCredentialAccount(s.ctx, s.repo, fresh, service.OpenAIOSWindows)
	s.Require().NoError(err)
	windows, err = s.repo.GetOpenAIOAuthOSCredential(s.ctx, account.ID, service.OpenAIOSWindows)
	s.Require().NoError(err)
...
func (s *AccountRepoSuite) TestOAuthOSCredentialsRestoreRetainsUnauthorizedDefault() {
