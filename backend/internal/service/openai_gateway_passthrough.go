...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	if account != nil && account.IsOpenAI() {
		...
		// This passthrough branch preserves its model (apart from the compact
	...
	var retryIdentityPlan *OpenAIOAuthIdentityPlan
	...
	for {
		...
		upstreamReq, buildErr := s.buildUpstreamRequestOpenAIPassthroughWithIdentityPlan(upstreamCtx, c, account, body, token, retryIdentityPlan)
		releaseUpstreamCtx()
		if buildErr != nil {
			return nil, buildErr
		}
		if retryIdentityPlan == nil && account.UsesOpenAICodexProtocol() {
			if resolved, ok := OpenAIOAuthIdentityPlanFromContext(c); ok {
				resolvedCopy := resolved
				retryIdentityPlan = &resolvedCopy
			}
		}
...
func (s *OpenAIGatewayService) buildUpstreamRequestOpenAIPassthrough(
	...
	return s.buildUpstreamRequestOpenAIPassthroughWithIdentityPlan(ctx, c, account, body, token, nil)
}

func (s *OpenAIGatewayService) buildUpstreamRequestOpenAIPassthroughWithIdentityPlan(
	ctx context.Context,
	c *gin.Context,
	account *Account,
	body []byte,
	token string,
	explicitPlan *OpenAIOAuthIdentityPlan,
) (*http.Request, error) {
	...
	if usesOpenAICodexIdentityProtocol(account) {
		...
		if isOpenAIResponsesCompactPath(c) {
			...
			if clientSessionID == "" && !identityModeEnabled {
		...
		if !identityPlan.TurnIdentityEnabled && !identityModeEnabled {
			if clientSessionID != "" {
...
func (s *OpenAIGatewayService) prepareOpenAIPassthroughCompactWindow(
	...
	if s == nil || c == nil || req == nil || account == nil || !account.UsesOpenAICodexProtocol() ||
...
func openAIStreamFailedEventSemanticStatus(payload []byte, message string) int {
	...
	switch {
	case code == "usage_limit_reached", code == "insufficient_quota",
		errType == "usage_limit_reached", errType == "insufficient_quota":
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	for documentScanner.Scan() {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			observeCodexTelemetryHTTPPayload(resp, dataBytes, rawEventType)
				...
				if !outputStarted {
					...
					if !cyberHit && !sawBareError {
						if status, errType, errMsg, matched := applyOpenAIStreamFailedErrorPassthroughRule(c, account.Platform, dataBytes, failedMessage); matched {
							...
							if marshalErr != nil {
								return resultWithUsage(), fmt.Errorf("marshal OpenAI passthrough error response: %w", marshalErr)
							}
							delivered := writeOpenAIResponseDataWithDelivery(c, status, "application/json; charset=utf-8", errorBody)
							if delivered && turnState != "" && !openAIPassthroughCompactWindowActive(c) {
								s.noteOpenAICodexTurnStateProvenance(c, account, turnState)
								turnStateCommitted = true
							}
