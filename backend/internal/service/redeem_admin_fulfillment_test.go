...
func TestAdminFulfillmentBypassesLimitAndKeepsRedeemAffiliate(t *testing.T) {
	...
	code := &RedeemCode{
		ID: 102, Code: "ADMIN-SUCCESS", Type: RedeemTypeBalance, Value: 20, GiftRatio: 25, GiftValue: 5, Status: StatusUnused,
	}
	...
	userRepo := &paymentFulfillmentGiftUserRepo{
		mockUserRepo: mockUserRepo{getByIDUser: &User{ID: userID}},
		creditWalletFn: func(txCtx context.Context, id int64, balance, gift float64) error {
			walletCalls++
			require.NotNil(t, dbent.TxFromContext(txCtx))
			require.Equal(t, userID, id)
			require.InDelta(t, 20, balance, 1e-8)
			require.InDelta(t, 5, gift, 1e-8)
			return nil
		},
	}
