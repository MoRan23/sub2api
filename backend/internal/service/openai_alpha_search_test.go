...
func TestForwardAlphaSearchOAuthPreservesWire(t *testing.T) {
	...
	expectedBody, err := sjson.SetRawBytes(body, "settings.user_location", []byte(`{"city":"Seattle","country":"US","region":"Washington","timezone":"America/Los_Angeles","type":"approximate"}`))
	require.NoError(t, err)
	...
	require.NoError(t, json.Unmarshal(metadata["turn_id"], &turnID))
	...
	require.True(t, ValidateFingerprintObservationUUIDv7(turnID))
	require.NotEqual(t, "search-turn", turnID)
...
func TestForwardAlphaSearchPATUsesResponsesWebSearchFallback(t *testing.T) {
	...
	fallbackPromptCacheKey := gjson.GetBytes(upstream.lastBody, "prompt_cache_key").String()
	require.NotEmpty(t, fallbackPromptCacheKey)
	require.Equal(t, upstream.lastReq.Header.Get("session-id"), fallbackPromptCacheKey)
	require.NotEqual(t, "responses-cache-key", fallbackPromptCacheKey)
