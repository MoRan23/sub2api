...
func TestOpenAICodexClientWindowPathsRestoreAfterIndependentCompacts(t *testing.T) {
	for _, transport := range []string{"http", "oauth_passthrough", "websocket", "websocket_http_bridge"} {
		t.Run(transport, func(t *testing.T) {
			...
			for range 2 {
				digest, err := OpenAICodexCompactTurnDigest("restore-path-secret", initialPlan.TurnIdentityNamespace, initialPlan.APIKeyID, current, codexClientWindowPathUUID(t))
			...
			// even when a different upstream credential handles the request.
			...
			outbound, restored := codexClientWindowPathBuild(t, transport, svc, upstream, &secondAccount, restoredContext, restoredBody)
			require.Equal(t, initialPlan.TurnIdentity, restored.TurnIdentity)
			require.Equal(t, initialPlan.WindowMappingKey, restored.WindowMappingKey)
			...
			retriedBody, retried := codexClientWindowPathBuild(t, transport, svc, upstream, &secondAccount, initialContext, initialBody)
