...
const rows = [
  ...
  { id: 4, user_id: 44, group_id: 2, status: 'active', user: { email: 'total@example.com' }, group: { subscription_type: 'total_quota' } }
]
...
describe('subscription bulk operations', () => {
  it('excludes total-quota subscriptions from adjustments but permits revocation', async () => {
    await select([4])
    expect(wrapper.get('[data-test="bulk-extend"]').attributes('disabled')).toBeDefined()
    expect(wrapper.get('[data-test="bulk-reset_quota"]').attributes('disabled')).toBeDefined()
    expect(wrapper.get('[data-test="bulk-revoke"]').attributes('disabled')).toBeUndefined()
    await select([1, 4])
    ...
    expect(form.text()).not.toContain('total@example.com')
  })
