...
func (e *PluginTransportError) Unwrap() error {
	if e == nil {
		return nil
	}
	return e.cause
...
func normalizePluginRPCError(ctx context.Context, operation string, err error, requestMayHaveBeenSent bool) error {
	...
	return &PluginTransportError{
		...
		// Keep cancellation recognizable without losing the no-replay marker
		// once request metadata may have reached the plugin/upstream.
		cause: contextErr,
	}
}
