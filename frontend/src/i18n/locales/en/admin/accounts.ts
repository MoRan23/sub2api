export default {
    accounts: {
      codexTurnState: {
        ...
        description: 'Lite HTTP business requests that are actually sent trigger parallel collection. Token and cookie packages are shared across systems for each credential account and final model in the system list. Requests continue without a cache. Disabling this feature stops cookie pooling, replacement and storage.',
        ...
        proxyHint: 'Background short requests use the dedicated proxies in list order. An empty list learns from business responses only. Changing the list does not immediately recollect a valid target cache.',
        useTicketProxy: 'Use ticket issuing proxy',
        bundleRoutingHint: 'Enabled by default: HTTP business requests using a valid package also use its issuing proxy. When disabled, compatible tickets and cookies are still injected, while business requests use the account’s original proxy. Without a usable package, requests use the original proxy without injecting that package. Business protocols are not converted; native WebSocket is unaffected.',
        bundleWireMode: 'Current package protocol',
        bundleProxy: 'Package issuing egress',
        bundleUnavailableReason: 'Current package unavailable reason',
        actualProxy: 'Actual egress for this request',
        routeSource: 'Egress selection for this request',
        lastEligibleCollection: 'Last business request eligible for collection',
        proxyDirect: 'Direct',
        diagnosticUnknown: 'Not recorded',
        wireMode: 'Request protocol',
        wireModes: { lite: 'Lite', responses: 'Responses', unknown: 'Not recorded' },
        routeSources: { account: 'Original account route', bundle: 'Package issuing route', collector: 'Collector configuration', unknown: 'Not recorded' },
        ...
        dotCollectorExpiring: 'Independently collected package expires within 30 seconds',
        dotCookieExpired: 'A cookie in the package expired; the package cannot be reused',
        dotExpiredIdle: 'Cache expired; collection stopped after 30 minutes without eligible business requests for this model',
        ...
        collectorOutboundHint: 'Independent collection carries neither an old turn-state nor old cookies. A returned target package is used by later business requests, so an omitted outbound state is expected.',
        ...
        sharedCacheHint: 'The token and cookie package for each credential account and model is shared across Windows, macOS and Linux. User-Agent, installation ID and session roots remain separate. Used only for HTTP outbound requests; native WebSocket requests neither use cached tokens nor trigger collection.',
        observationSystemHint: 'The system filter below selects historical observations only; it does not change the shared cache on the left.',
        ...
        collectionStatuses: { idle: 'Idle', scheduled: 'Scheduled refresh', pending: 'Pending', collecting: 'Collecting', backoff: 'Waiting to retry', paused: 'Paused', blocked: 'Blocked' },
        ...
        lifetimePolicyHint: 'Tokens are locally valid for 240 seconds from issue time. Eligible models collect again 30 seconds after success, with a 12-second attempt limit, and stop after 30 minutes without a Lite HTTP business send. Packages are reusable until the earliest token or cookie expiry. These are local policies, not confirmed upstream lifetimes.',
        ...
        cookieSendStates: { carried: 'Cookie present in sending headers', not_carried: 'Cookie explicitly absent from sending headers', not_sent: 'Request not sent', unknown: 'Not recorded; Cookie presence unknown' },
        cookieSendEvidenceHint: 'Based on the header snapshot at the actual HTTP send boundary, not confirmation of upstream receipt. Token length does not establish Cookie presence.',
        cookieSource: 'Cookie source',
        cookieNames: 'Cookie names',
        cookieExpiresAt: 'Cookie expiry',
        cookieSession: 'Session cookie (no separate expiry declared)',
        cookieUnknownExpiry: 'Expiry not recorded',
        cookieSources: { none: 'No Cookie source selected', bundle: 'This request’s token package', persistent: 'Persistent pool', memory: 'Process session pool', mixed: 'Persistent and process session pools', unknown: 'Not recorded' },
        cookieReasons: {
          cookie_scope_missing: 'Authorization identity missing; cookie pool not used', cookie_invalid_scope: 'Authorization identity invalid; cookie pool not used',
          cookie_host_not_allowed: 'Target host is outside cookie scope', cookie_store_unavailable: 'Cookie storage unavailable; request forwarded normally',
          cookie_store_corrupt: 'Cookie data could not be read and was not used', cookie_stale_scope: 'Authorization changed; previous cookie pool not used',
          cookie_empty: 'No usable cookies', cookie_sent: 'Pool cookies sent', cookie_updated: 'Cookie pool updated',
          cookie_staged: 'Response cookies are staged, awaiting target-token confirmation for this response',
          cookie_target_required: 'This auxiliary request has no target-token confirmation; the cookie pool was not updated',
          cookie_target_rejected: 'The target token was not accepted; staged cookies were discarded and the cookie pool was not updated',
          cookie_commit_conflict: 'The cookie pool was updated concurrently; this result did not overwrite newer data',
          cookie_attempt_closed: 'This request attempt ended or was abandoned; staged cookies were not committed',
          cookie_bundle_expired: 'The token package expired and was not reused',
          cookie_bundle_invalid: 'The token package failed validation and was not reused',
          cookie_snapshot_unavailable: 'This HTTP attempt did not capture a complete cookie snapshot; no token package was published',
          cookie_bundle_saved: 'Cookies were saved atomically with this target token',
          unknown: 'Cookie status not classified'
        },
        ...
        expiresAt: 'Local token expiration',
        bundleExpiresAt: 'HTTP package reusable until',
        bundleRemaining: 'Remaining package lifetime',
        remaining: '{seconds} seconds remaining',
        lastBusiness: 'Last business request',
        lastCollected: 'Last collection',
        nextCollect: 'Earliest next collection',
        ...
        reasons: {
          bundle_protocol_mismatch: 'Request protocol differs from the package; package bypassed',
          bundle_binding_invalid: 'Package binding failed validation; package bypassed',
          bundle_proxy_unavailable: 'The package’s issuing proxy is unavailable; package bypassed',
          bundle_proxy_changed: 'The package’s issuing proxy settings changed; package bypassed',
          bundle_route_changed: 'The package’s issuing route changed; package bypassed',
          bundle_rebuild_baseline: 'Original business request restored; package bypassed',
          waiting_eligible_business: 'Waiting for a business request eligible for collection',
          ...
          cookie_expired: 'A cookie in the token package expired',
          business_active: 'A Lite HTTP business send triggered collection', refresh: 'Scheduled refresh for an eligible model',
          ...
          authorization_unavailable: 'Account OAuth authorization unavailable',
        ...
        reasonHints: {
          bundle_protocol_mismatch: 'Lite and Responses packages are not interchangeable. This request follows its original business route.',
          bundle_binding_invalid: 'The package’s protocol or egress binding could not be confirmed. It is not reused and does not select an unknown route.',
          bundle_proxy_unavailable: 'Check that the package’s issuing proxy still exists and is available. This request follows its original business route.',
          bundle_proxy_changed: 'The issuing proxy settings changed, so the old package can no longer use that binding. This request follows its original business route.',
          bundle_route_changed: 'The issuing route no longer matches the saved binding. This request bypasses the old package and follows its original business route.',
          bundle_rebuild_baseline: 'Validation before sending required bypassing the package. The original business request and its route were restored.',
          waiting_eligible_business: 'A Lite HTTP business send starts or extends independent collection, even without a package. Ordinary Responses traffic and prepared requests that were never sent do not extend collection.',
          cookie_expired: 'The current package can no longer be reused in full. Business requests continue normally and active models will collect again. This does not mean the upstream token has expired.',
          refresh: 'Collection resumes 30 seconds after the last success and stops after 30 minutes without a Lite HTTP business send for this model. This is a scheduled refresh, not an error backoff.',
          ...
          collector_connect_timeout: 'The attempt timed out while connecting. Check proxy reachability and connection speed. The overall collection limit remains 12 seconds.',
          collector_tls_timeout: 'The attempt timed out during the TLS handshake. Check the collector proxy and handshake speed. The overall collection limit remains 12 seconds.',
          collector_response_header_timeout: 'The request was sent, but response headers did not arrive in time. Check upstream response speed and the proxy connection. The overall collection limit remains 12 seconds.',
          ...
          collection_timeout: 'The connection or response timed out; each collection attempt has a 12-second limit. Check the collector proxy and upstream connection speed.',
          ...
          authorization_unavailable: 'Valid account OAuth authorization could not be obtained, so cache maintenance is paused. Check the account authorization status and reauthorize if needed.',
      ...
      openai: {
        ...
        osProfilesDescription: 'All three systems share one account authorization while keeping separate User-Agent, installation ID and session roots. Requests with an unknown system use the default.',
        identityOS: 'Request identity system',
        accountAuthorization: 'Account OAuth authorization (shared across systems)',
        ...
        revokeAuthorizationConfirm: 'Revoke this account’s OAuth authorization? Requests for all three systems will stop until the account is authorized again.',
        authorizationSaved: 'Account configuration updated',
        authorizationSessionLocked: 'This authorization link is bound to the account. Close and reopen to authorize a different account.',
        importedAuthorizationHint: 'All three systems will use the imported account credentials. Their installation identities remain unchanged.',
        exportOSDescription: 'Download the account’s shared auth.json for use on Windows, macOS or Linux.',
