...
func TestProxyOpenAIWSHTTPBridgeTurn_SeparatesRequestedAndObservedServiceTier(t *testing.T) {
	...
	// policy。本测试只覆盖局部 observer：canonical 请求 priority 与上游
	// response.completed service_tier=default 必须分开保存。
...
func TestProxyOpenAIWSHTTPBridgeTurn_FastAliasRemainsSeparateFromObservedDefault(t *testing.T) {
	gin.SetMode(gin.TestMode)

	// 客户端别名 fast 规范为 priority，上游回显 default 独立保存。
	sse := strings.Join([]string{
