...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	parseClientPayload := func(turn int, raw []byte) (openAIWSClientPayload, error) {
		...
		// OAuth-like payloads stay in their pre-Lite shape until the shared
		// finalizer resolves the mapped model's authoritative manifest capability.
	...
	sendAndRelay := func(turn int, lease *openAIWSConnLease, payload []byte, payloadBytes int, originalModel string, imageBillingModel string, imageSizeTier string, imageInputSize string, requestedReasoningEffort *string) (*OpenAIForwardResult, error) {
		...
		if account.UsesOpenAICodexProtocol() {
			// Retries, replay repair, and continuation recovery may mutate the
			// request after its initial projection. Re-run the complete pure
			// projector (including the provenance guard) with the same frozen plan
			// at the final physical-send boundary.
			projected, projectErr := s.projectOpenAIOAuthWSFrame(c, account, pinnedIdentityPlan, payload)
			if projectErr != nil {
				return nil, wrapOpenAIWSIngressTurnError("write_upstream_metadata", projectErr, false)
			}
			payload = projected
	...
	for {
		...
		if account.UsesOpenAICodexProtocol() && !pinnedIdentityModeEnabled {
