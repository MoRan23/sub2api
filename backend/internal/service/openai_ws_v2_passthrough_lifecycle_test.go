...
func TestPassthroughLifecycle_FirstTurnSemanticRateLimitKeepsBodyClassification(t *testing.T) {
	...
	svc.openaiWSPassthroughDialer.(*stagedPassthroughDialer).headers = successfulOpenAIWSQuotaHeaders()
	...
	upstream.Send(`{"type":"error","error":{"code":"rate_limit_exceeded","type":"rate_limit_error","message":"try again"}}`)
	select {
	case err := <-serverErr:
		...
		require.Empty(t, repo.resetTimes, "successful handshake headers are not a semantic 429 quota signal")
	case <-time.After(3 * time.Second):
		t.Fatal("first-turn semantic rate limit did not terminate passthrough")
	}
}
