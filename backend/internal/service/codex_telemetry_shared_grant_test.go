...
func TestCodexTelemetrySharedGrantRefreshKeepsThreeIdentityPools(t *testing.T) {
	...
	for _, family := range []string{"windows", "macos", "linux"} {
		profile := accounts.account.OpenAIOAuthOSProfiles.Profiles[family]
		attempt := runtimeTestAttempt(now, family, "same-turn", "same-sampling")
		attempt.profile.input.OwnerAccountID, attempt.poolKey.OwnerAccountID = accounts.account.ID, accounts.account.ID
		attempt.profile.input.InstallationID, attempt.poolKey.InstallationID = profile.InstallationID, profile.InstallationID
		attempt.profile.input.UserAgent = profile.UserAgent
	...
	for _, token := range []string{"current-shared-token", "refreshed-shared-token"} {
		accounts.account.Credentials = map[string]any{"access_token": token, "chatgpt_account_id": "workspace"}
		...
		for _, family := range []string{"windows", "macos", "linux"} {
			...
			job.profile.input.InstallationID = accounts.account.OpenAIOAuthOSProfiles.Profiles[family].InstallationID
