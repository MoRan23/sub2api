...
type transferWriter struct {
	...
	lowercaseHeaderNames bool  // HTTP/1.1 wire names only; never mutate Header/Trailer
...
func (t *transferWriter) writeBody(w io.Writer, dumps []*dump.Dumper) (err error) {
	...
	if chunked(t.TransferEncoding) {
		...
		if t.Trailer != nil {
			trailers := t.Trailer
			if t.lowercaseHeaderNames {
				trailers = make(http.Header, len(t.Trailer))
				kvs, sorter := headerSortedKeyValues(t.Trailer, nil)
				for _, kv := range kvs {
					key := strings.ToLower(kv.Key)
					trailers[key] = append(trailers[key], kv.Values...)
				}
