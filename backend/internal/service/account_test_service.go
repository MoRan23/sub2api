...
func AppendOpenAIConfiguredTestModels(models []openai.Model, account *Account) []openai.Model {
	...
	if !account.IsOpenAIPassthroughEnabled() {
		for requested := range mapping {
			...
			// Wildcards are routing rules, not concrete test model names.
	...
	for _, requested := range requestedModels {
		...
		models = append(models, openai.Model{ID: requested, Object: "model", Type: "model", OwnedBy: "openai", DisplayName: openaiCodexDisplayName(requested)})
...
// generateSessionStringForUserAgent uses the request's frozen client version so
// metadata.user_id remains consistent with its outbound User-Agent.
func generateSessionStringForUserAgent(userAgent string) (string, error) {
	b := make([]byte, 32)
...
func createTestPayload(modelID string) (map[string]any, error) {
	return createTestPayloadForUserAgent(modelID, claude.DefaultHeaders()["User-Agent"])
}

func createTestPayloadForUserAgent(modelID, userAgent string) (map[string]any, error) {
	sessionID, err := generateSessionStringForUserAgent(userAgent)
	if err != nil {
		return nil, err
	}
...
func (s *AccountTestService) testClaudeAccountConnection(c *gin.Context, account *Account, modelID string) error {
	...
	clientHeaders := claude.DefaultHeaders()
	payload, err := createTestPayloadForUserAgent(testModelID, clientHeaders["User-Agent"])
	if err != nil {
...
func (s *AccountTestService) testGrokRealtime(c *gin.Context, ctx context.Context, account *Account, authToken, modelID string) error {
	...
	conn, status, _, dialErr := dialer.Dial(withOpenAIWSTransportAccount(dialCtx, account.ID), wsURL, headers, s.grokTestProxyURL(account))
	if dialErr != nil {
