...
func (s *GeminiMessagesCompatService) AntigravityGeminiModelIDs(ctx context.Context, groupID *int64, requireMixed bool) ([]string, error) {
	...
	for _, account := range accounts {
		...
		addModel := func(model string) {
			if isAntigravityGeminiModel(model) && IsSafeGeminiModelPathSegment(model) && strings.TrimSpace(mapAntigravityModel(&account, model)) != "" {
				seen[model] = struct{}{}
			}
		}
		for model := range account.GetModelMapping() {
			if strings.Contains(model, "*") {
				// Mapping patterns are routing rules, not callable model IDs. Expand
				// them against known models while respecting the account's final mapping.
				for candidate := range domain.DefaultAntigravityModelMapping {
