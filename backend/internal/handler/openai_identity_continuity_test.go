...
func TestOpenAIResponsesHandlerPreservesIdentityAcrossResponsesAndCompact(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Empty(t, requests[0].Header.Get("x-codex-installation-id"))
			require.Equal(t, openAIIdentityContinuityInstallationID, gjson.GetBytes(requests[0].Body, "client_metadata.x-codex-installation-id").String())
