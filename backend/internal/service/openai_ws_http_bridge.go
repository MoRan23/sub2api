...
func (s *OpenAIGatewayService) proxyOpenAIWSHTTPBridgeTurn(
	...
	if account.IsOpenAIApiKey() && isOpenAIResponsesLiteWebSocketPayload(payload) {
	...
	buildUpstreamRequest := func(requestBody []byte) (*http.Request, error) {
		...
		if account.Platform == PlatformGrok {
			...
			if bridgeResponsesLite && c != nil && c.Request != nil {
				// Carry the frozen WS capability into the HTTP finalizer without
			...
			upstreamReq, buildErr = s.buildUpstreamRequestOpenAIPassthroughWithIdentityPlan(
				upstreamCtx,
				c,
				account,
				requestBody,
		...
		// The passthrough builder clears observation provenance before its final
		// projection. Restore the frozen plan and observe each physical attempt.
