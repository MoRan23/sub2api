    ...
    settings: {
      ...
      features: {
        ...
        affiliate: {
          ...
          enabledHint: 'When off, the affiliate menu is hidden, the aff parameter is ignored at signup, and new recharges generate no rebate. Existing rebate balances can still be transferred to gift balance.',
          adminRechargeRebate: 'Rebate Admin Deposits',
          adminRechargeRebateHint: 'When enabled, only ordinary balance added through User Management > Deposit generates affiliate rebates. Gift balance, setting a balance, and withdrawing funds do not.',
      ...
      gatewayForwarding: {
        ...
        grokBaseURLModeEUWest1: 'Regional API (eu-west-1)',
	        ...
	        fingerprintUnification: 'OAuth SDK fingerprint unification (X-Stainless)',
	        fingerprintUnificationHint: 'Unifies X-Stainless-* headers across users sharing one OAuth account. It is independent from Codex installation_id, UUIDv7 session, and UA normalization. Disabling passes through each client\'s original headers.',
        ...
        openaiCodexVersionAutoSyncHint: 'Fetches the latest stable client version from the official repository every 6 hours. Disabling it only stops future refreshes; without a manual version, the last valid synced version remains in use, falling back to the built-in version when none exists.',
        ...
        openaiOAuthDailySessionRotation: 'Daily OpenAI OAuth session rotation',
        openaiOAuthDailySessionRotationHint: 'Creates three streaming roots and one account sync root per OAuth credential each day, with API key and logical-session stickiness. Disabled by default.',
      ...
      codexFingerprintNormalization: {
        ...
        paused: 'Globally paused. Values below are retained and take effect when re-enabled; UA safety pairing still keeps the required client identity fields consistent.',
        installationTitle: 'Fixed installation_id',
        installationHint: 'Globally permits each OAuth parent account to use its own fixed UUIDv4. Manage the per-account switch and UUID in account details; passthrough mode continues to preserve the client value.',
        manageAccounts: 'Manage accounts',
        clientIdentityTitle: 'UA / Originator / Version',
        clientIdentityHint: 'When enabled, all three are normalized as one Codex client identity. When disabled, safety pairing still rejects invalid or conflicting combinations instead of blindly passing them through.',
      },
      webSearchEmulation: {
      ...
      payment: {
        ...
        balanceGiftRatio: 'Recharge Gift Ratio',
        balanceGiftRatioHint: 'Adds gift balance as a percentage of ordinary credited balance. Enter 0–100%, up to 4 decimal places.',
        balanceGiftPreview: 'Preview: every 100 USD of ordinary balance adds {gift} USD of gift balance',
