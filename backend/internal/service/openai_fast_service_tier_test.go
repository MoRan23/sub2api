...
func TestForward_ServiceTierFilteredByPolicyIsNotRaisedByObservedPriority(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Body: io.NopCloser(strings.NewReader(
			`{"id":"resp_1","object":"response","status":"completed","model":"gpt-5.5","service_tier":"priority","output":[],"usage":{"input_tokens":1,"output_tokens":1}}`,
		)),
	}}
	...
	require.NoError(t, err)
	require.NotNil(t, result)
	// 出站 body 已剥离 service_tier；即使上游回显 priority，也不得升级计费。
	require.False(t, gjson.GetBytes(upstream.lastBody, "service_tier").Exists(),
