      ...
      <template #table>
        <div class="flex h-full min-h-[320px] flex-col">
          ...
          <div class="min-h-0 flex-1 overflow-auto">
            ...
            <ul v-else-if="sessions.length > 0" class="divide-y divide-gray-100 dark:divide-dark-700">
              <li
              ...
              >
                ...
                >
                  <button
                    ...
                    :aria-controls="sessionPanelId(sessionIndex)"
                  ...
                  <div class="min-w-0 space-y-1">
                    <div class="flex min-w-0 items-center gap-1.5 text-sm font-medium text-gray-900 dark:text-white">
                      <Icon name="user" size="xs" class="shrink-0 text-gray-400" />
                      <span class="truncate" :title="session.email || session.username">
                        {{ userLabel(session) }}
                      </span>
                      <span v-if="session.user_id > 0" class="shrink-0 text-xs font-normal text-gray-400">
                        #{{ session.user_id }}
                      </span>
                    </div>
                    <div class="flex min-w-0 items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
                      <Icon name="key" size="xs" class="shrink-0" />
                      <span class="truncate" :title="session.api_key_name">
                        {{ apiKeyLabel(session) }}
                      </span>
                      <span v-if="session.api_key_id > 0" class="shrink-0">#{{ session.api_key_id }}</span>
                    </div>
                  </div>
                ...
                <div
                  ...
                  :id="sessionPanelId(sessionIndex)"
                ...
                >
                  ...
                  >
                    <div class="grid grid-cols-[2rem_minmax(0,1fr)_minmax(0,1.35fr)_auto] items-center gap-3 py-3 pl-9 pr-4">
                      <button
                        type="button"
                        ...
                        :aria-controls="threadPanelId(sessionIndex, threadIndex)"
                    ...
                    <div
                      ...
                      :id="threadPanelId(sessionIndex, threadIndex)"
...
function sessionPanelId(index: number): string {
  return `fingerprint-session-${index}`
}

function threadPanelId(sessionIndex: number, threadIndex: number): string {
  return `fingerprint-session-${sessionIndex}-thread-${threadIndex}`
}
...
function userLabel(session: FingerprintObservationSessionNode): string {
  return session.username || session.email || t('admin.fingerprintObservation.unknownUser')
}

function apiKeyLabel(session: FingerprintObservationSessionNode): string {
  return session.api_key_name || t('admin.fingerprintObservation.unknownApiKey')
}
