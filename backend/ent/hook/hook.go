...
// The UserSubscriptionQuotaEventFunc type is an adapter to allow the use of ordinary
// function as UserSubscriptionQuotaEvent mutator.
type UserSubscriptionQuotaEventFunc func(context.Context, *ent.UserSubscriptionQuotaEventMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f UserSubscriptionQuotaEventFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.UserSubscriptionQuotaEventMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.UserSubscriptionQuotaEventMutation", m)
}
