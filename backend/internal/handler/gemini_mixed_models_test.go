...
func TestGeminiNativeModelsExpandsWildcardMappings(t *testing.T) {
	for _, pattern := range []string{"gemini-3*", "*"} {
		t.Run(pattern, func(t *testing.T) {
			...
			repo := &geminiAllowlistAccountRepoStub{gatewayModelsAccountRepoStub: gatewayModelsAccountRepoStub{byGroup: map[int64][]service.Account{
				groupID: {{ID: 1, Platform: service.PlatformAntigravity,
					...
					Credentials: map[string]any{"model_mapping": map[string]any{
						pattern:                "gemini-3.1-pro-high",
						"gemini-synced-custom": "gemini-3.8-flash-high",
						"gemini-disabled":      "",
						"gemini-invalid/path":  "gemini-3.8-flash-high",
					}},
				}},
			}}}
			h := &GatewayHandler{geminiCompatService: service.NewGeminiMessagesCompatService(repo, nil, nil, nil, nil, nil, nil, nil, nil)}
			...
			for _, model := range got.Models {
				names = append(names, model.Name)
				require.True(t, service.IsSafeGeminiModelPathSegment(strings.TrimPrefix(model.Name, "models/")), model.Name)
			}
			require.Contains(t, names, "models/gemini-3-flash")
			require.Contains(t, names, "models/gemini-3.8-flash-high")
			require.Contains(t, names, "models/gemini-synced-custom")
			require.NotContains(t, names, "models/"+pattern)
			require.NotContains(t, names, "models/gemini-disabled")
			if pattern == "gemini-3*" {
				require.NotContains(t, names, "models/gemini-2.5-pro")
			} else {
