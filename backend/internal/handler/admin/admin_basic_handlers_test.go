...
func TestUserHandlerEndpoints(t *testing.T) {
	...
	rec = httptest.NewRecorder()
	req = httptest.NewRequest(http.MethodPost, "/api/v1/admin/users/1/balance", bytes.NewBufferString(`{"balance":0,"operation":"set"}`))
	req.Header.Set("Content-Type", "application/json")
	router.ServeHTTP(rec, req)
	require.Equal(t, http.StatusOK, rec.Code)

	rec = httptest.NewRecorder()
	req = httptest.NewRequest(http.MethodPost, "/api/v1/admin/users/1/balance", bytes.NewBufferString(`{"balance":1,"gift_balance":1,"operation":"set"}`))
	req.Header.Set("Content-Type", "application/json")
	router.ServeHTTP(rec, req)
