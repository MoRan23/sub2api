...
func TestContentModerationEngineWhitelistAuditKeepsPolicyAndEvidence(t *testing.T) {
	for _, engine := range []string{ContentModerationEngineOpenAI, ContentModerationEngineTypeSafe} {
		for _, fail := range []bool{false, true} {
			...
			t.Run(name, func(t *testing.T) {
				...
				server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
					calls.Add(1)
					require.Equal(t, "Bearer "+engine+"-key", r.Header.Get("Authorization"))
					if fail {
					...
					if engine == ContentModerationEngineTypeSafe {
						var request typesafe.Request
						require.NoError(t, json.NewDecoder(r.Body).Decode(&request))
						require.Equal(t, "audit me", request.State)
						answers := map[string]any{}
						for _, category := range ContentModerationCategories() {
							answers[category] = map[string]any{"type": "noul", "noul": 0.99}
						}
						require.NoError(t, json.NewEncoder(w).Encode(map[string]any{"model": "jev-test", "answers": answers}))
					} else {
						require.NoError(t, json.NewEncoder(w).Encode(moderationAPIResponse{Model: "omni-test", Results: []moderationAPIResult{{CategoryScores: map[string]float64{"sexual": 0.99}}}}))
					}
				}))
				defer server.Close()
				cfg := defaultContentModerationConfig()
				...
				svc := &ContentModerationService{
					...
					repo:        repo, hashCache: hashCache, httpClient: server.Client(), asyncQueue: make(chan contentModerationTask, 1),
				}
				decision, err := svc.Check(context.Background(), ContentModerationCheckInput{UserID: 1001, Protocol: ContentModerationProtocolOpenAIResponses,
					Body: []byte(`{"input":[{"role":"user","content":"audit me"}]}`)})
				require.NoError(t, err)
				require.True(t, decision.Allowed)
				require.Equal(t, ContentModerationActionWhitelistAllow, decision.Action)
				task := <-svc.asyncQueue
				require.Nil(t, task.input.Body)
				require.Equal(t, []string{engine + "-key"}, task.config.apiKeys())
				svc.runAuditOnlyWithKeywordMatch(context.Background(), task.input, task.config, task.content, task.matchedKeyword, task.keywordExcerpt, task.inputHash, nil)
				require.Equal(t, int32(1), calls.Load())
				logs := repo.snapshotLogs()
				require.Len(t, logs, 1)
				require.Equal(t, !fail, logs[0].Flagged)
				require.Equal(t, ContentModerationActionWhitelistAllow, logs[0].Action)
				require.NotNil(t, logs[0].EngineMeta)
				require.Equal(t, engine, logs[0].EngineMeta.Engine)
...
func TestContentModerationEngineWhitelistReminderKeywordAudit(t *testing.T) {
	for _, engine := range []string{ContentModerationEngineOpenAI, ContentModerationEngineTypeSafe} {
		t.Run(engine, func(t *testing.T) {
			...
			svc := &ContentModerationService{
				settingRepo: &contentModerationTestSettingRepo{values: map[string]string{SettingKeyRiskControlEnabled: "true", SettingKeyContentModerationConfig: string(raw)}},
				repo:        repo, hashCache: hashCache, asyncQueue: make(chan contentModerationTask, 1),
			}
			decision, err := svc.Check(context.Background(), ContentModerationCheckInput{UserID: 1001, Protocol: ContentModerationProtocolAnthropicMessages,
				Body: []byte(`{"messages":[{"role":"user","content":"<system-reminder>secret-token</system-reminder>"}]}`)})
			require.NoError(t, err)
			require.True(t, decision.Allowed)
			require.Equal(t, ContentModerationActionWhitelistAllow, decision.Action)
			task := <-svc.asyncQueue
			require.Nil(t, task.input.Body)
			require.True(t, task.content.IsEmpty())
			require.Equal(t, "secret-token", task.matchedKeyword)
			svc.runAuditOnlyWithKeywordMatch(context.Background(), task.input, task.config, task.content, task.matchedKeyword, task.keywordExcerpt, task.inputHash, nil)
			logs := repo.snapshotLogs()
			require.Len(t, logs, 1)
...
func TestContentModerationAuditOnlyReminderEvidenceIsBounded(t *testing.T) {
	cfg := defaultContentModerationConfig()
	cfg.BlockedKeywords = []string{"secret-token"}
	input := ContentModerationCheckInput{Protocol: ContentModerationProtocolAnthropicMessages}
	input.Body = reminderTestBody(t, input.Protocol, []string{"<system-reminder>" + strings.Repeat("a", maxModerationInputRunes+1) + " secret-token</system-reminder>"})
	content := ExtractContentModerationInput(input.Protocol, input.Body)
	require.True(t, content.IsEmpty())
	svc := &ContentModerationService{asyncQueue: make(chan contentModerationTask, 1)}
	svc.enqueueAuditOnly(input, cfg, content, content.Hash())
	task := <-svc.asyncQueue
	require.Nil(t, task.input.Body)
	require.Equal(t, "secret-token", task.matchedKeyword)
	require.LessOrEqual(t, len([]rune(task.keywordExcerpt)), maxModerationExcerptRunes)
}
