...
func TestApplyOAuthCredentialsPreservesExistingNonAuthCredentials(t *testing.T) {
	...
	stub := &completedOpenAIOAuthAdmin{adminOpenAIRefreshPersistence: &adminOpenAIRefreshPersistence{initial: account}}
	handler := NewAccountHandler(stub, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil)
	router := gin.New()
	router.POST("/accounts/:id/apply-oauth-credentials", handler.ApplyOAuthCredentials)
	recorder := httptest.NewRecorder()
	...
	require.Equal(t, http.StatusOK, recorder.Code)
	require.Equal(t, 1, stub.bindCalls)
	require.Zero(t, stub.fullUpdates, "OpenAI reauthorization must use the atomic account credential binder")
	require.Equal(t, map[string]any{
		"access_token": "new-token", "refresh_token": "new-refresh-token",
	}, stub.bound, "the handler must not merge stale token fields into the completed authorization")
	require.Equal(t, map[string]any{
		"access_token":  "new-token",
		"refresh_token": "new-refresh-token",
		"model_mapping": map[string]any{"gpt-5": "gpt-5"},
		"account_id":    "existing-account-id",
	}, stub.initial.Credentials)
}
