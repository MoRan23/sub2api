...
func TestGrokVideoStickySelectionIgnoresHealthEscape(t *testing.T) {
	...
	selection, capacitySpillover, err := scheduler.selectBySessionHash(context.Background(), req)
	require.NoError(t, err)
	require.Nil(t, selection)
	require.False(t, capacitySpillover, "health escape is not a temporary capacity spillover")
	req.DisableStickyEscape = true
	selection, capacitySpillover, err = scheduler.selectBySessionHash(context.Background(), req)
	require.NoError(t, err)
	require.False(t, capacitySpillover)
	require.True(t, selection.Acquired)
	selection.ReleaseFunc()
}
