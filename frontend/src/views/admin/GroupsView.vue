        ...
        >
          ...
          <template #cell-billing_type="{ row }">
            <div class="space-y-1">
              ...
              >
                <div
                  v-if="row.subscription_type === 'total_quota'"
                  class="flex flex-wrap items-center gap-x-1 gap-y-0.5"
                >
                  <span v-if="row.total_limit_usd" class="whitespace-nowrap"
                    >{{ formatUsd(row.total_limit_usd) }}/{{
...
// 监听 subscription_type 变化，订阅类分组 is_exclusive 默认为 true；非订阅模式清空不适用配置
watch(
...
// 编辑表单：切换类型时清空不适用配置，避免残留随更新请求提交被后端拒绝
watch(
