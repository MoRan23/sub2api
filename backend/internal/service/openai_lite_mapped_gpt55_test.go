...
func TestMappedGPT55LiteCompatibility(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			capabilities := effectiveCodexHTTPModelCapabilities(a, tc.model, CodexModelCapabilities{}, true)
			require.Equal(t, tc.wantLite, capabilities.UseResponsesLite)
			got := body
			if !capabilities.UseResponsesLite {
				var err error
				got, err = applyOpenAIResponsesLiteWSMarker(body, false)
