...
func TestImportCodexExistingDifferentOSRequiresVerifiedRefresh(t *testing.T) {
	...
	require.Contains(t, result.Errors[0].Message, "refresh_token verification")
	require.Empty(t, admin.updatedAccounts)
	require.Empty(t, admin.createdAccounts)
	require.Equal(t, access, admin.accounts[0].Credentials["access_token"])
}

func TestImportCodexNewAccountBindsOnlySelectedInitialOS(t *testing.T) {
