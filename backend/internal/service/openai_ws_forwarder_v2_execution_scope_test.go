...
type executionScopeHandshakeDialer struct {
	openAIWSQueueDialer
	handshake http.Header
}

func (d *executionScopeHandshakeDialer) Dial(ctx context.Context, target string, headers http.Header, proxy string) (openAIWSClientConn, int, http.Header, error) {
	conn, status, _, err := d.openAIWSQueueDialer.Dial(ctx, target, headers, proxy)
...
// Forward 会在转发前按账号身份改写 client_metadata。执行作用域必须取自客户端原始请求，
// 避免固定根与出站身份改写影响不同会话的状态隔离，并保持 HTTP 与 WS 接入路径的键一致。
func TestOpenAIGatewayService_Forward_WSv2_ExecutionScopeUsesOriginalIdentity(t *testing.T) {
	...
	pool.setClientDialerForTest(&executionScopeHandshakeDialer{
	...
	plan, ok := OpenAIOAuthIdentityPlanFromContext(cB)
	require.True(t, ok)
	require.NotEmpty(t, plan.TurnIdentity.ThreadID)
	injectedScope, _ := deriveOpenAISessionHashes(fmt.Sprintf("openai_ws_exec:%d|thread=%s", apiKeyID, plan.TurnIdentity.ThreadID))
