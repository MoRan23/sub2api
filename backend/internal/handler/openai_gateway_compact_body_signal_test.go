	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Equal(t, service.OpenAIEndpointCapabilityRemoteCompactionV2,
				openAIResponsesRequiredCapabilityForRequest(false, openAICompactionRouteFromContext(c), service.PlatformOpenAI))
			capture, captured := service.OpenAIOAuthIdentityCaptureFromContext(c)
			require.True(t, captured)
			require.Equal(t, "pck-signal-1", capture.Logical.SessionKey)
			require.Len(t, capture.Aliases, 1, "native v2 must not add the legacy compact endpoint alias")
...
func TestOpenAIResponsesCompactionRoutingFlags(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantCapabilityAfter: service.OpenAIEndpointCapabilityRemoteCompactionV2,
