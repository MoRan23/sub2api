...
func (s *OpenAIGatewayService) forwardAsRawChatCompletions(
	...
	// Keep the final outbound tier (after policy filter/force) separate from the
	// observed response tier so usage recording can apply credential and pricing rules.
	...
	customUA := account.GetOpenAIOutboundUserAgent()
