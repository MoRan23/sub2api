...
// Matcher is an immutable, concurrency-safe Aho-Corasick substring matcher.
// Matches are case-insensitive and the earliest configured keyword wins.
type Matcher struct {
	nodes           []node
	edges           []edge
...
func New(keywords []string) *Matcher {
		...
		for _, label := range []byte(strings.ToLower(keyword)) {
			...
			if next < 0 {
				...
				buildEdges = append(buildEdges, buildEdge{
					target: next, nextSibling: firstBuildEdge(buildNodes[state]), label: label,
				})
	...
	for nodeIndex := range buildNodes {
		...
		for edgeIndex := firstBuildEdge(buildNodes[nodeIndex]); edgeIndex >= 0; edgeIndex = buildEdges[edgeIndex].nextSibling {
			build := buildEdges[edgeIndex]
			outgoing[count] = edge{target: build.target, label: build.label}
			count++
		}
