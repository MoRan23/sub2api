...
type UpdateAccountRequest struct {
	...
	OpenAIEnvironmentFingerprint *string        `json:"openai_environment_fingerprint"`
...
func (h *AccountHandler) Update(c *gin.Context) {
	...
	account, err := h.adminService.UpdateAccount(ctx, accountID, &service.UpdateAccountInput{
		...
		Notes:                        req.Notes,
		Type:                         req.Type,
		OpenAIEnvironmentFingerprint: req.OpenAIEnvironmentFingerprint,
		Credentials:                  req.Credentials,
		Extra:                        req.Extra,
		ProxyID:                      req.ProxyID,
...
func (h *AccountHandler) refreshSingleAccount(ctx context.Context, account *service.Account) (*service.Account, string, error) {
	...
	var expectedOpenAIAccount *service.Account

	if account.IsOpenAI() {
		expectedOpenAIAccount = snapshotAdminOpenAIRefreshAccount(account)
	...
	var err error
	if expectedOpenAIAccount != nil {
		var applied bool
		updatedAccount, applied, err = h.adminService.PersistOpenAIOAuthRefreshCredentials(ctx, expectedOpenAIAccount, newCredentials)
		if applied {
			invalidateAdminOpenAIRefreshToken(ctx, h.tokenCacheInvalidator, expectedOpenAIAccount)
		}
		if err == nil && !applied {
	...
	if h.tokenCacheInvalidator != nil && expectedOpenAIAccount == nil {
...
func (h *AccountHandler) ApplyOAuthCredentials(c *gin.Context) {
	...
	if accountAuthorization {
		...
		// Other providers keep their existing account configuration alongside the
		// freshly authorized fields. OpenAI's binder merges only configuration in
		// its locked transaction, so stale token fields cannot revive old auth.
		req.Credentials = service.SanitizeStoredCredentials(existing.Platform, service.MergeCredentials(existing.Credentials, req.Credentials))
