...
// Per-account Codex installation_id pinning (OpenAI OAuth parent accounts only),
// persisted in accounts.extra. When pinning is enabled the gateway emits the
// system-generated account UUID on every outbound request and ignores whatever
// the inbound client reported.
const (
	...
	// openAIInstallationRotateEnabledKey is accepted only as a legacy cleanup
	// key. Rotation is no longer supported and incoming values are discarded.
...
func (a *Account) UsesOpenAICodexProtocol() bool {
	return a != nil && (a.IsOpenAIOAuthLike() ||
		(a.Type == AccountTypeOAuth && strings.TrimSpace(a.Platform) == ""))
}
...
func (a *Account) GetOpenAIOutboundUserAgent() string {
	...
	return resolveCodexClientIdentityPlan(CodexClientIdentitySafePair, a.GetOpenAIUserAgent()).UserAgent
}
...
// inbound client value). OpenAI credentials using the Codex inference protocol
// are eligible. Default is ON:
...
func (a *Account) IsOpenAIInstallationPinEnabled() bool {
	if !isOpenAICodexInstallationOwner(a) {
		return false
	}
	if a.Extra == nil {
		return true
...
// was generated by the service. Empty means the account needs repair.
