...
func (s *OpenAIGatewayService) forwardWithOpenAIHTTPBundle(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if account.IsOpenAIOAuthLike() {
		finalModel := s.resolveOpenAIResponsesHTTPFinalModel(c, account, gjson.GetBytes(body, "model").String())
		reasoningBody, reasoningChanged, reasoningErr := normalizeOpenAIResponsesReasoningMode(body, finalModel)
		if reasoningErr != nil {
	...
	if responsesLite && account.IsOpenAIApiKey() {
		liteBody, changed, liteErr := normalizeOpenAIResponsesLitePayloadForAccount(body, account)
		if liteErr != nil {
			writeOpenAIResponsesLiteValidationError(c, liteErr)
			return nil, liteErr
		}
		if changed {
			body = liteBody
	...
	// OAuth-like Responses Lite is finalized after model mapping so a known
	// manifest value remains authoritative over an inbound capability marker.
	...
	if imageGenerationAllowed && !isCompactRequest && (codexImageGenerationBridgeEnabled || isOpenAIImageGenerationModel(requestView.Model) || openAIRequestBodyImageGenerationToolNeedsNormalization(body) || isOpenAIImageGenerationModel(upstreamModel)) {
		...
		if codexImageGenerationBridgeEnabled {
			if usesOpenAICodexIdentityProtocol(account) && applyDefaultCodexInstructions(decoded, upstreamModel) {
				markDecodedModified()
			}
	...
	for {
		...
		if reqStream {
			...
			completeCodexTelemetryHTTPResponse(resp, err)
			if err != nil {
...
func (s *OpenAIGatewayService) buildUpstreamRequestWithOptions(
	...
	if usesOpenAICodexIdentityProtocol(account) {
		...
		// signals are present. Resolve the final logical seed once so UUIDv7 mode
		// does not create an unused second mapping in the identity store.
		compactSession := ""
		if isOpenAIResponsesCompactPath(c) {
		...
		if !plan.TurnIdentityEnabled && !identityModeEnabled {
			...
			if isOpenAIResponsesCompactPath(c) {
				...
				// resolved seed is empty (Header.Get remains empty either way).
