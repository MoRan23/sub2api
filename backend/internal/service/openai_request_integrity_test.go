...
func TestOpenAIRequestIntegrityLocationAdditionRequiresFrozenSource(t *testing.T) {
	...
	changed := bytes.ReplaceAll(prepared, []byte(`"Washington"`), []byte(`"Ohio"`))
	require.NotEqual(t, string(prepared), string(changed), "the tampered location must differ from the frozen target")
