export default {
    settings: {
      ...
      codexTurnStateModels: {
        ...
        formatHint: '每行填写一个完整模型 ID，区分大小写且精确匹配，不支持前缀或通配符。默认：gpt-6-astra、gpt-5.6-sol。留空将暂停所有模型的缓存维护。',
      ...
      features: {
        ...
        affiliate: {
          ...
          enabled: '启用邀请返利',
          enabledHint: '关闭后用户菜单中的邀请页面入口隐藏、注册时忽略邀请码、新充值不再产生返利。已有返利额度仍可转入赠送余额。',
          adminRechargeRebate: '管理员充值参与返利',
          adminRechargeRebateHint: '开启后，仅通过“用户管理 > 充值”增加的普通余额会产生邀请返利；赠送余额、设置余额和扣款不参与返利。',
          rebateRate: '全局返利比例',
      ...
      gatewayForwarding: {
        ...
        grokBaseURLModeEUWest1: '区域 API（eu-west-1）',
	        ...
	        fingerprintUnification: 'OAuth SDK 指纹统一（X-Stainless）',
	        fingerprintUnificationHint: '统一共享同一 OAuth 账号的用户的 X-Stainless-* 请求头；与 Codex installation_id、会话 UUIDv7 和 UA 指纹归一无关。关闭后透传客户端原始请求头。',
        ...
        openaiCodexVersionAutoSyncHint: '每 6 小时从官方仓库获取最新稳定版客户端版本号。关闭后只停止后续刷新；未手填版本时继续使用最后一次有效同步值，没有同步值时使用内置版本。',
        ...
        openaiOAuthDailySessionRotation: '每日固定根会话',
        openaiOAuthDailySessionRotationHint: '每个 OAuth 账号每天生成三条流式根和一条同步根；流式请求按 API Key 与逻辑会话粘性分配，跨业务日重新生成。默认关闭。',
      ...
      openaiRequestPolicies: {
        ...
        timezoneTitle: '统一请求时区与搜索地域',
        timezoneHint: '按账号本次实际使用的代理出口 IP 设置支持的环境时区与搜索地域；直连时使用当前服务实例的出口 IP。地域未知时使用 US / Washington / Seattle / America/Los_Angeles，不阻断请求。支持补齐未提供的搜索地域；本轮日期按网关首次接收时刻换算，同一出口重试保持不变，历史日期保留。不修改服务器全局时区。',
        passthroughTitle: '统一透传请求时区与搜索地域',
        passthroughHint: '同时处理透传请求中的支持位置。仅在请求时区与搜索地域总开关开启时生效；关闭总开关仍会保留此配置。',
      ...
      codexFingerprintNormalization: {
        ...
        paused: '当前全局暂停。下方配置会原样保留，重新开启后生效；UA 安全配对仍会保留必要的客户端身份一致性。',
        installationTitle: '固定 installation_id',
        installationHint: '全局允许 OAuth 母账号使用各自固定的 UUIDv4。账号是否固定及具体 UUID 在账号详情中管理；透传模式仍保留客户端原值。',
        manageAccounts: '前往账号管理',
        clientIdentityTitle: 'UA / Originator / Version',
        clientIdentityHint: '开启时将三者归一为同一 Codex 客户端身份；关闭时仍执行安全配对，不会无条件透传非法或相互冲突的组合。',
      },
      webSearchEmulation: {
        title: 'Web Search 模拟',
      ...
      payment: {
        ...
        balanceGiftRatio: '充值赠送比例',
        balanceGiftRatioHint: '按普通到账余额的百分比额外赠送，范围 0–100%，最多 4 位小数',
        balanceGiftPreview: '预览：每到账 100 USD 普通余额，额外赠送 {gift} USD 赠送余额',
