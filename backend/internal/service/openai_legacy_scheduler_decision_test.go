...
func TestLegacySchedulerDecision_PreviousResponsePreservesDurableSessionBinding(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
	for _, movable := range []bool{false, true} {
		...
		require.NoError(t, svc.setStickySessionAccountID(ctx, &groupID, "durable-session", 38101, time.Hour))
		require.NoError(t, svc.getOpenAIWSStateStore().BindResponseAccount(ctx, groupID, "resp_spillover", 38102, time.Hour))

		selection, decision, err := svc.SelectAccountWithSchedulerForCapability(
			ctx, &groupID, "resp_spillover", "durable-session", "gpt-5.1", nil,
			OpenAIUpstreamTransportAny, OpenAIEndpointCapabilityChatCompletions, false, movable, true,
		)
		require.NoError(t, err)
		require.NotNil(t, selection)
		releaseLegacySchedulerDecisionSelection(selection)
		...
		bound, err := svc.getStickySessionAccountID(ctx, &groupID, "durable-session")
		require.NoError(t, err)
		require.Equal(t, int64(38101), bound, "a temporary response owner must not rewrite durable session affinity")
	}
}

func TestLegacySchedulerDecision_HardPreviousResponseChecksChannelBeforeSlot(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
	ctx := context.Background()
	groupID := int64(38121)
	...
	svc.channelService = newTestChannelService(makeStandardRepo(Channel{
		ID: 38122, Status: StatusActive, GroupIDs: []int64{groupID}, RestrictModels: true,
	...
	require.NoError(t, svc.getOpenAIWSStateStore().BindResponseAccount(ctx, groupID, "resp_restricted", 38102, time.Hour))

	selection, _, err := svc.SelectAccountWithSchedulerForCapability(ctx, &groupID, "resp_restricted", "", "gpt-5.1", nil,
		OpenAIUpstreamTransportAny, OpenAIEndpointCapabilityChatCompletions, false, false, true)
