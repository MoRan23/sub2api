...
func TestContentModerationEngineHandlerRoundTrip(t *testing.T) {
	...
	body := `{"engine":"typesafe","engine_configs":{"typesafe":{"base_url":"https://api.typesafe.ai","model":"jev-latest","api_keys":["new-secret"],"thresholds":{"sexual":0.91}}},"content_moderation_whitelist_user_ids":[17],"cyber_policy_whitelist_user_ids":[23],"cyber_policy_notification_emails":["ops@example.com"],"cyber_policy_exclude_from_ban_count":true}`
	...
	require.Contains(t, w.Body.String(), `"content_moderation_whitelist_user_ids":[17]`)
	require.Contains(t, w.Body.String(), `"cyber_policy_whitelist_user_ids":[23]`)
	require.Contains(t, w.Body.String(), `"cyber_policy_notification_emails":["ops@example.com"]`)
	require.Contains(t, w.Body.String(), `"cyber_policy_exclude_from_ban_count":true`)
