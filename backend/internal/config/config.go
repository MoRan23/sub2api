...
type GatewayConfig struct {
	...
	// Deprecated: retained only so old configuration can be detected and
	// warned about. Runtime policy is owned by Codex fingerprint settings.
	DisableCodexIdentityEnforcement bool `mapstructure:"disable_codex_identity_enforcement"`
	// Deprecated: retained only for startup diagnostics.
...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	if cfg.Gateway.DisableCodexIdentityEnforcement || cfg.Gateway.DisableCodexOriginatorNormalization {
		slog.Warn("deprecated Codex identity disable config is ignored; use Codex fingerprint normalization settings")
		cfg.Gateway.DisableCodexIdentityEnforcement = false
		cfg.Gateway.DisableCodexOriginatorNormalization = false
	}
