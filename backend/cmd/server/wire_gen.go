...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	store := repository.NewOpenAIHTTPCookieStore(db, secretEncryptor)
	manager := openaicookies.NewManager(store)
	httpUpstream := repository.NewHTTPUpstreamWithCookies(configConfig, manager)
	...
	openAIOAuthClient := repository.NewOpenAIOAuthClientWithCookies(manager)
	privacyClientFactory := providePrivacyClientFactory(manager)
	openAIOAuthService := service.ProvideOpenAIOAuthService(proxyRepository, openAIOAuthClient, privacyClientFactory, settingService, accountRepository, manager)
	...
	oAuthSyncSessionRepository := repository.NewOpenAIOAuthSyncSessionRepository(client)
	oAuthDailySessionRepository := repository.NewOpenAIOAuthDailySessionRepository(client)
	...
	openAIQuotaService := service.ProvideOpenAIQuotaService(accountRepository, proxyRepository, openAITokenProvider, privacyClientFactory, openAIGatewayService, settingService)
	usageCache := service.NewUsageCache()
	accountUsageService := service.ProvideAccountUsageService(accountRepository, usageLogRepository, claudeUsageFetcher, geminiQuotaService, antigravityQuotaFetcher, grokQuotaFetcher, grokQuotaService, openAIQuotaService, usageCache, identityCache, tlsFingerprintProfileService, openAIGatewayService, settingService)
	...
	openAIOAuthHandler := admin.ProvideOpenAIOAuthHandler(openAIOAuthService, adminService, openAIQuotaService, rateLimitService, oAuthDailySessionRepository, settingService, codexTelemetryService)
	...
	groupApplicationRepository := repository.NewGroupApplicationRepository(db)
	groupApplicationService := service.NewGroupApplicationService(groupApplicationRepository, settingRepository, secretEncryptor)
	groupApplicationWorker := service.ProvideGroupApplicationWorker(groupApplicationRepository, groupApplicationService, emailService)
	...
	handlerGroupApplicationHandler := handler.NewGroupApplicationHandler(groupApplicationService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, handlerGroupApplicationHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
	...
	openAIOAuthOSProfileBackfill := service.ProvideOpenAIOAuthOSProfileBackfill(accountRepository)
	proxyExpiryService := service.ProvideProxyExpiryService(proxyRepository)
	subscriptionExpiryService := service.ProvideSubscriptionExpiryService(userSubscriptionRepository, settingRepository, notificationEmailService, leaderLockCache, db)
	batchImageWorkerRuntime := service.ProvideBatchImageWorkerRuntime(batchImageRepository, accountRepository, batchImageQueue, usageBillingRepository, usageLogRepository, batchImageModelPricingResolver, apiKeyAuthCacheInvalidator, configConfig)
	openAIOutboundSessionV1CleanupWorker := service.ProvideOpenAIOutboundSessionV1CleanupWorker(redisClient)
