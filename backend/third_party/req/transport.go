...
type Transport struct {
	...
	// LowercaseHTTP1HeaderNames writes HTTP/1.1 header and trailer names in
	// lowercase without changing the request's Header or Trailer maps. It is
	// disabled by default and does not affect HTTP/2 or HTTP/3. Configure it
	// before the transport is used; like other transport options, it must not
	// be changed concurrently with requests.
	LowercaseHTTP1HeaderNames bool

	// CancelDialOnRequestCancel makes connection establishment follow request
	// cancellation, including proxy negotiation and TLS handshakes. By default,
	// a canceled request's pending dial may finish for use by another request.
	// Established connections keep their normal pooling behavior. Configure this
	// option before using the transport.
	CancelDialOnRequestCancel bool
...
// SetLowercaseHTTP1HeaderNames controls HTTP/1.1 wire header name casing.
// The option is disabled by default and must be set before sending requests.
func (t *Transport) SetLowercaseHTTP1HeaderNames(enabled bool) *Transport {
	t.LowercaseHTTP1HeaderNames = enabled
	return t
}

// SetCancelDialOnRequestCancel controls whether a pending dial follows request
// cancellation. It is disabled by default and must be set before sending requests.
func (t *Transport) SetCancelDialOnRequestCancel(enabled bool) *Transport {
	t.CancelDialOnRequestCancel = enabled
	return t
}
...
func (t *Transport) Clone() *Transport {
	tt := &Transport{
		LowercaseHTTP1HeaderNames: t.LowercaseHTTP1HeaderNames,
		CancelDialOnRequestCancel: t.CancelDialOnRequestCancel,
...
func (t *Transport) getConn(treq *transportRequest, cm connectMethod) (pc *persistConn, err error) {
	...
	if t.CancelDialOnRequestCancel {
		// The transport context also includes legacy Request.Cancel and
		// Transport.CancelRequest signals, while retaining request values.
		dialParent = treq.ctx
	}
	dialCtx, dialCancel := context.WithCancel(dialParent)
...
func (pc *persistConn) writeRequest(r *http.Request, w io.Writer, usingProxy bool, extraHeaders http.Header, waitForContinue func() bool) (err error) {
	...
	_writeHeader := func(key string, values ...string) error {
		if pc.t.LowercaseHTTP1HeaderNames {
	...
	tw.lowercaseHeaderNames = pc.t.LowercaseHTTP1HeaderNames
	...
	if sort { // sort and write headers
		...
		for _, kv := range kvs {
			if err := _writeHeader(kv.Key, kv.Values...); err != nil {
				return err
			}
		}
	}
