...
账号管理页的常规 OpenAI OAuth 账号可通过操作菜单导出 `auth.json`。文件用于 Codex 的 ChatGPT 登录凭据存储；它不包含本项目的账号配置、UA、installation ID、每日会话根或 turn-state 运行态。

## 适用账号与读取行为

- 仅支持 OpenAI 平台的常规 OAuth 账号。
- API Key、setup-token、Personal Access Token、agent identity 和影子账号不可导出。影子账号应前往凭据母账号操作。
- 每次导出直接读取数据库最新完整账号快照，保持该次读取中的凭据一致。导出不刷新 token、不修改账号、不调用上游接口，也不进行真实模型请求。
- 导出要求非空 `access_token`，以及可被 Codex ID-token 解析结构读取的 `id_token`。这里只验证 JWT 包装与相关字段类型，不验证签名或授权资格；ID token 的旧签发时间、过期时间不会单独阻止导出。

## 管理接口与文件格式

接口为 `GET /api/v1/admin/accounts/:id/codex-auth`，沿用管理员认证和现有敏感操作 step-up 策略。需要 step-up 时，前端完成验证后重试；取消验证不会生成下载。
...
导出允许携带已过期 access token 并给出警告；导入继续保留原有有效期校验，因此已过期凭据不保证能够重新导入。文件不会复制服务端私有凭据代次或运行态。
