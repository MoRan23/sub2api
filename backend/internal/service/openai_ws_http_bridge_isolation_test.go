...
type httpBridgeIsolationStateStore struct {
	OpenAIWSStateStore
	recoveryCalls atomic.Int64
}

func (s *httpBridgeIsolationStateStore) GetSessionTurnState(int64, string) (string, bool) {
	s.recoveryCalls.Add(1)
	return "unexpected-shared-turn-state", true
}

func (s *httpBridgeIsolationStateStore) BindSessionTurnState(int64, string, string, time.Duration) {
	s.recoveryCalls.Add(1)
}

func (s *httpBridgeIsolationStateStore) DeleteSessionTurnState(int64, string) {
	s.recoveryCalls.Add(1)
}

func (s *httpBridgeIsolationStateStore) GetSessionConn(int64, string) (string, bool) {
	s.recoveryCalls.Add(1)
	return "unexpected-shared-connection", true
}

func (s *httpBridgeIsolationStateStore) BindSessionConn(int64, string, string, time.Duration) {
	s.recoveryCalls.Add(1)
}

func (s *httpBridgeIsolationStateStore) GetResponseConn(string) (string, bool) {
	s.recoveryCalls.Add(1)
	return "unexpected-response-connection", true
}
...
func TestOpenAIWSHTTPBridgeSessionIsolationAcrossSameSessionHash(t *testing.T) {
	...
	stateStore := &httpBridgeIsolationStateStore{OpenAIWSStateStore: NewOpenAIWSStateStore(nil)}
	...
	stateStore.OpenAIWSStateStore.BindSessionTurnState(groupID, seedHash, "sentinel-turn-state", time.Hour)
	stateStore.OpenAIWSStateStore.BindSessionConn(groupID, seedHash, "sentinel-native-conn", time.Hour)
	...
	require.Zero(t, stateStore.recoveryCalls.Load(), "HTTP bridges must not read or publish shared native-WS recovery state")
	gotState, ok := stateStore.OpenAIWSStateStore.GetSessionTurnState(groupID, seedHash)
	require.True(t, ok)
	require.Equal(t, "sentinel-turn-state", gotState)
	gotConn, ok := stateStore.OpenAIWSStateStore.GetSessionConn(groupID, seedHash)
	require.True(t, ok)
