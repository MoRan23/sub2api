...
func TestGroupHandlerSimpleModeIgnoresTotalQuotaFields(t *testing.T) {
	svc := newStubAdminService()
	r := newSimpleModeGroupRouter(svc)
	body := `{"name":"basic","platform":"anthropic","subscription_type":"total_quota","total_limit_usd":75}`
	for _, method := range []string{http.MethodPost, http.MethodPut} {
		...
		require.NotContains(t, res.Body.String(), "total_limit_usd")
	}
	require.Len(t, svc.createdGroups, 1)
	require.Equal(t, service.SubscriptionTypeStandard, svc.createdGroups[0].SubscriptionType)
	require.Nil(t, svc.createdGroups[0].TotalLimitUSD)
	require.Len(t, svc.updatedGroups, 1)
	require.Empty(t, svc.updatedGroups[0].SubscriptionType)
	require.Nil(t, svc.updatedGroups[0].TotalLimitUSD)
}
...
func TestGroupHandlerSimpleModeResponseUsesFieldAllowlist(t *testing.T) {
	...
	svc.groups = []service.Group{{
		...
		TotalLimitUSD: float64PtrForSimpleModeTest(75),
	}}
	r := newSimpleModeGroupRouter(svc)
	res := httptest.NewRecorder()
	...
	for _, forbidden := range []string{
		...
		"audio_realtime_price_per_min", "model_routing", "total_limit_usd",
	} {
