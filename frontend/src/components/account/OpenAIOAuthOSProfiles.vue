<template>
  <div data-testid="openai-os-profiles" class="mt-4 space-y-3">
    ...
    <p class="text-sm text-gray-700 dark:text-gray-200">
      {{ t('admin.accounts.openai.defaultOS') }}: {{ profiles?.default_os ? osLabels[profiles.default_os] : creating ? 'Windows' : '—' }}
    </p>
...
defineProps<{ profiles?: OpenAIOAuthOSProfiles; creating?: boolean; inherited?: boolean; disabled?: boolean; regenerating?: OpenAIOAuthOS | null }>()
defineEmits<{ regenerate: [os: OpenAIOAuthOS] }>()
const { t } = useI18n()
