...
func (s *OpenAIGatewayService) forwardAsAnthropic(
	...
	// A streaming handler may return billable usage together with a terminal
	// error. Keep success-only continuation mutations gated, but always attach
	// the final outbound tier and reasoning metadata to a non-nil usage result.
	if result != nil {
		if handleErr == nil {
		...
		// Keep the final outbound request tier separate from the response-observed
		// tier already captured by the response handler.
