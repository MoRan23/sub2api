...
func TestUpdateCredentialsOpenCodeGoIdentityChangeClearsManagedExtra(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)WITH previous_profile.*UPDATE accounts.*- 'opencode_go_usage_auto_refresh'.*- 'opencode_go_usage_snapshot'.*- 'ollama_cloud_usage_session'`).
...
func TestUpdateCredentialsOpenCodeGoToOllamaCrossOverClearsManagedExtra(t *testing.T) {
	client, mock := newOllamaCloudUsageRepositoryTestClient(t)
	mock.ExpectBegin()
	mock.ExpectQuery(`(?s)WITH previous_profile.*UPDATE accounts.*opencode_go_usage_auto_refresh.*opencode_go_usage_snapshot`).
...
func TestUpdateCredentialsOpenCodeGoCleanupRequiresChangedCredentials(t *testing.T) {
	client, mock := newOllamaCloudUsageRepositoryTestClient(t)
	mock.ExpectBegin()
	mock.ExpectQuery(`(?s)WITH previous_profile.*UPDATE accounts.*CASE.*AND credentials IS DISTINCT FROM \$1::jsonb`).
...
func TestBulkUpdateOpenCodeGoEligiblePredicateIncludesBaseURL(t *testing.T) {
	...
	require.NotEmpty(t, exec.execQueries)
	query := normalizeSQLWhitespace(exec.execQueries[0])
	// Locate the OpenCode eligibility branch inside the fork's outer managed
	// identity guard, rather than assuming it is the query's first CASE.
	caseStart := strings.Index(query, "type = 'apikey' AND ((platform = 'opencode_go'")
	require.NotEqual(t, -1, caseStart)
	firstThen := strings.Index(query[caseStart:], " THEN ")
	require.NotEqual(t, -1, firstThen)
	firstWhen := query[caseStart : caseStart+firstThen]
...
func TestUpdateCredentialsOpenCodeBranchPrecedesOllamaBranch(t *testing.T) {
	client, mock := newOllamaCloudUsageRepositoryTestClient(t)
	mock.ExpectBegin()
	mock.ExpectQuery(`(?s)WITH previous_profile.*UPDATE accounts.*platform = 'opencode_go'.*\[oO\]\[lL\]\[lL\]\[aA\]\[mM\]\[aA\]`).
		WithArgs(`{"api_key":"new-key","base_url":"https://opencode.ai/zen/go/v1"}`, int64(17)).
		WillReturnRows(sqlmock.NewRows([]string{"profile_eligibility_changed"}).AddRow(false))
	mock.ExpectExec(regexp.QuoteMeta("INSERT INTO scheduler_outbox")).
		WithArgs(service.SchedulerOutboxEventAccountChanged, int64(17), nil, nil, sqlmock.AnyArg()).
