...
func TestUsageLogRepositoryGetAllGroupUsageSummarySnapshotFailures(t *testing.T) {
	for _, stage := range []string{"begin", "watermark", "summary", "commit"} {
		t.Run(stage, func(t *testing.T) {
			db, mock := newSQLMock(t)
			repo := newUsageLogRepositoryWithSQL(nil, db)
			useGroupUsageRepositoryTestTimezone(t, "UTC")
			failure := errors.New("snapshot " + stage + " failed")

			begin := mock.ExpectBegin()
			if stage == "begin" {
				begin.WillReturnError(failure)
			} else {
				watermark := mock.ExpectQuery(`(?s)COUNT\(\*\).*usage_group_rollup_state.*WHERE id = 1`)
				if stage == "watermark" {
					watermark.WillReturnError(failure)
					mock.ExpectRollback()
				} else {
					watermark.WillReturnRows(sqlmock.NewRows([]string{"count", "closed_before", "retained_from", "timezone_name"}).
						AddRow(1, "2026-03-07", time.Date(2026, 2, 1, 0, 0, 0, 0, time.UTC), "UTC"))
					summary := mock.ExpectQuery(`(?s)usage_group_daily_rollups.*FROM usage_logs ul`)
					if stage == "summary" {
						summary.WillReturnError(failure)
						mock.ExpectRollback()
					} else {
						summary.WillReturnRows(sqlmock.NewRows([]string{"group_id", "total_cost", "today_cost", "yesterday_cost"}).
							AddRow(int64(7), 12.5, 1.25, 2.5))
			...
			result, err := repo.GetAllGroupUsageSummary(context.Background(), time.Date(2026, 3, 9, 0, 0, 0, 0, time.UTC))
