...
func TestOpenAIWSIngressSessionPreemptionUsesFrozenCanonicalIdentity(t *testing.T) {
	...
	require.True(t, armed)
	defer firstCleanup()
	// Reconnect ownership is now scoped to the declared thread, not the shared
	// root session. The original body still wins over stale session headers.
	expectedHash, _ := deriveOpenAISessionHashes("openai_ws_exec:11|thread=canonical-thread")
