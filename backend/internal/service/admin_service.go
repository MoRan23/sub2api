...
type AdminService interface {
	...
	UpdateUserBalance(ctx context.Context, userID int64, balance, giftBalance float64, operation string, notes string) (*User, error)
	...
	PersistOpenAIOAuthRefreshCredentials(ctx context.Context, expected *Account, credentials map[string]any) (*Account, bool, error)
	RegenerateOpenAIInstallationID(ctx context.Context, id int64) (string, error)
...
type UpdateAccountInput struct {
	...
	OpenAIEnvironmentFingerprint *string
	Credentials                  map[string]any
	Extra                        map[string]any
	ProxyID                      *int64
	Concurrency                  *int     // 使用指针区分"未提供"和"设置为0"
...
func NewAdminService(
	...
	if len(channelCacheInvalidators) > 0 {
		channelCacheInvalidator = channelCacheInvalidators[0]
	}
