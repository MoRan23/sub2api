...
describe('useOpenAIOAuth.exchangeAuthCode', () => {
  it('ignores a previous account authorization URL that resolves after reset', async () => {
    ...
    const oauth = useOpenAIOAuth()
    const pending = oauth.generateAuthUrl(undefined, undefined, 42)
    oauth.resetState()
  ...
  it('binds an existing account without selecting a system when generating authorization', async () => {
    ...
    const oauth = useOpenAIOAuth()
    await oauth.generateAuthUrl(5, undefined, 42)
    expect(adminAPI.accounts.generateAuthUrl).toHaveBeenLastCalledWith('/admin/openai/generate-auth-url', { proxy_id: 5, account_id: 42, purpose: 'authorize' })
    expect(oauth.boundAccountId.value).toBe(42)
    oauth.resetState()
    expect(oauth.boundAccountId.value).toBeNull()
  })

  it('sends manual refresh-token imports to the shared account grant', async () => {
    vi.mocked(adminAPI.accounts.refreshOpenAIToken).mockResolvedValueOnce({ account: { id: 42 }, os: 'macos' })
    const oauth = useOpenAIOAuth()
    const result = await oauth.validateRefreshToken('synthetic-rt', 5, undefined, 42)
    expect(adminAPI.accounts.refreshOpenAIToken).toHaveBeenLastCalledWith('synthetic-rt', 5, '/admin/openai/refresh-token', undefined, 42)
