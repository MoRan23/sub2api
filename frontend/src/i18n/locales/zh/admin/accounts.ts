export default {
    accounts: {
      codexTurnState: {
        ...
        description: '正常业务响应优先获取，按凭据账号与最终上游模型复用；仅系统生效模型名单内的模型维护缓存，无缓存时照常转发。',
        accountType: '账号分类',
        ...
        dotCollectorExpiring: '独立采集缓存将在 30 秒内过期',
        ...
        collectorOutboundHint: '独立采集不携带旧 turn-state；取得的目标状态供之后的业务请求使用，出站未携带属于正常行为。',
        ...
        collectionStatuses: { idle: '空闲', pending: '等待采集', collecting: '正在采集', backoff: '等待重试', paused: '已暂停', blocked: '暂不可采集' },
        ...
        lifetimePolicyHint: '本地有效期按签发时间起 240 秒计算，提前 30 秒续采；这是本项目的缓存策略，不代表上游已确认的有效期。',
        loading: '正在加载状态…',
        ...
        cookieSession: '进程会话（无明确到期时间）',
        cookieUnknownExpiry: '未记录到期时间',
        cookieSources: { none: '未携带', persistent: '持久化池', memory: '进程会话池', mixed: '持久化与进程会话池', unknown: '未记录' },
        cookieReasons: {
        ...
        reasonHints: {
          ...
          collector_connect_timeout: '在建立连接阶段超时。请检查代理连通性和出口连接速度，单次采集仍受 20 秒总时限约束。',
          collector_tls_timeout: '在 TLS 握手阶段超时。请检查采集代理和出口的握手速度，单次采集仍受 20 秒总时限约束。',
          collector_response_header_timeout: '请求已发送，但等待响应头超时。请检查上游响应速度及代理链路，单次采集仍受 20 秒总时限约束。',
          ...
          collection_timeout: '连接或响应等待超时，单次采集上限为 20 秒。请检查采集代理与上游连接速度。',
