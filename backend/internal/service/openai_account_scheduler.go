...
func (s openAISelectionFilterStats) summary(extra string) string {
	...
	if s.pool > 0 && s.reasons["oauth_authorization_unavailable"] == s.pool {
		_, _ = b.WriteString(", all_candidates_missing_oauth_authorization")
	}
	return b.String()
}
...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatibleReason(ctx context.Context, account *Account, req OpenAIAccountScheduleRequest) (bool, string) {
	...
	if !openAIAccountOSAuthorizationEligible(ctx, account, func(id int64) *Account {
		...
		return false, "oauth_authorization_unavailable"
	}
