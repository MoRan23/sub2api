export default {
    accounts: {
      codexTurnState: {
        ...
        cookieReasons: { cookie_scope_missing: '缺少授权身份，未使用 Cookie 池', cookie_invalid_scope: '授权身份无效，未使用 Cookie 池', cookie_host_not_allowed: '目标主机不在 Cookie 适用范围', cookie_store_unavailable: 'Cookie 存储暂不可用，本次照常转发', cookie_store_corrupt: 'Cookie 数据无法读取，本次未使用', cookie_stale_scope: '授权已变化，未使用旧 Cookie 池', cookie_empty: '没有可用 Cookie', cookie_sent: '已携带池内 Cookie', cookie_updated: '已更新 Cookie 池', unknown: 'Cookie 状态未分类' },
      ...
      openai: {
        ...
        codexFingerprintCreateDesc: '创建后自动生成 Windows、macOS、Linux 三套固定安装身份。首次授权的系统成为默认系统，其他系统需分别授权。',
        ...
        osProfilesDescription: '每个系统分别保存授权与安装身份；安装配置存在不代表已授权。无法识别请求系统时使用默认系统。',
        authorizationOS: '授权系统',
        authorizationRequired: '请先完成该系统的授权。',
        ...
        revokeOS: '解除授权',
        setDefaultOS: '设为默认系统',
        revokeOSConfirm: '解除 {os} 的授权？该系统的请求将停止，直到重新完成授权。',
        authorizationSaved: '系统授权已更新',
        authorizationSessionLocked: '此授权链接已固定账号与系统。若要更换，请关闭后重新开始授权。',
        importedAuthorizationHint: '将现有凭据导入所选系统；导入已有凭据不代表完成了另一系统的新 OAuth 登录。',
        exportOSDescription: '选择已授权系统，下载文件仅包含该系统的一份 auth.json。',
        defaultOS: '默认系统',
