...
func (s *referralClientStub) QueryEligibility(ctx context.Context, call OpenAIReferralCall) (*OpenAIReferralEligibility, error) {
	s.calls = append(s.calls, call)
	s.contexts = append(s.contexts, ctx)
	return s.eligibility, s.queryErr
}
func (s *referralClientStub) SendInvite(ctx context.Context, call OpenAIReferralCall, email string) error {
	s.calls = append(s.calls, call)
	s.contexts = append(s.contexts, ctx)
...
func TestOpenAIReferralPreservesAuxiliaryIdentityAndPolicy(t *testing.T) {
	client := &referralClientStub{eligibility: &OpenAIReferralEligibility{}}
	svc, repo := referralTestService(t, "plus", client)
	account := repo.accounts[100]
	account.Credentials["access_token"] = "test-token"
	account.Credentials["user_agent"] = "codex-tui/0.155.1 (Ubuntu 22.4.0; x86_64) xterm-256color"
	...
	_, err := svc.QueryReferralEligibility(ctx, account.ID)
	require.NoError(t, err)
	require.Len(t, client.contexts, 1)
	scope, ok := codexnative.ScopeFromContext(client.contexts[0])
	require.True(t, ok)
	require.Equal(t, account.ID, scope.AccountID)
	require.Equal(t, account.GetOpenAIUserAgent(), scope.AccountUserAgent)
	frozen, ok := openai.RequestPolicyFromContext(client.contexts[0])
	require.True(t, ok)
	require.False(t, frozen.CodexResidencyUS)
	require.Equal(t, proxy.URL(), client.calls[0].ProxyURL)
	require.Empty(t, client.calls[0].Headers["x-codex-turn-state"])
	require.Empty(t, client.calls[0].Headers["cookie"])
	require.Empty(t, client.calls[0].Headers["session_id"])
}
