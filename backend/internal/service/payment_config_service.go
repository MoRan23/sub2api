...
const (
	...
	SettingBalanceGiftRatio    = "BALANCE_GIFT_RATIO"
...
type PaymentConfig struct {
	...
	BalanceGiftRatio          float64  `json:"balance_gift_ratio"`
...
func (s *PaymentConfigService) GetPaymentConfig(ctx context.Context) (*PaymentConfig, error) {
	keys := []string{
		...
		SettingEnabledPaymentTypes, SettingBalancePayDisabled, SettingBalanceRechargeMult, SettingBalanceGiftRatio, SettingSubscriptionUSDToCNYRate, SettingRechargeFeeRate, SettingLoadBalanceStrategy,
...
func (s *PaymentConfigService) parsePaymentConfig(vals map[string]string) *PaymentConfig {
	cfg := &PaymentConfig{
		...
		BalanceGiftRatio:          normalizeGiftRatio(pcParseFloat(vals[SettingBalanceGiftRatio], 0)),
...
func (s *PaymentConfigService) UpdatePaymentConfig(ctx context.Context, req UpdatePaymentConfigRequest) error {
	...
	if req.BalanceGiftRatio != nil {
		if _, ok := quantizeGiftRatio(*req.BalanceGiftRatio); !ok {
	...
	if req.BalanceGiftRatio != nil {
		m[SettingBalanceGiftRatio] = formatGiftRatio(req.BalanceGiftRatio)
	}
	if req.SubscriptionUSDToCNYRate != nil {
...
func formatGiftRatio(v *float64) string {
	if v == nil {
		return ""
	}
	ratio, ok := quantizeGiftRatio(*v)
	if !ok {
		return ""
	}
